...
Chi-based HTTP router for the MirageNet dashboard API
...
func (s *Server) buildRouter() *chi.Mux {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Route("/attackers", func(r chi.Router) {
			...
			r.Get("/{ip}/risk", s.handleAttackerRisk)
			r.Get(
				"/{ip}/risk/history",
				s.handleAttackerRiskHistory,
			)
		})

		r.Route("/risk", func(r chi.Router) {
			r.Get("/high", s.handleHighRiskAttackers)
			r.Get("/stats", s.handleRiskStats)
		})
