...
type RiskScorer interface {
	ComputeScore(profile *types.AttackerRiskProfile) *types.RiskScore
}
...
type DataStore interface {
	...
	GetAttackerRiskProfile(ctx context.Context, attackerIP string) (*types.AttackerRiskProfile, error)
	SaveRiskScore(ctx context.Context, attackerIP string, score *types.RiskScore, profile *types.AttackerRiskProfile) error
	UpdateAttackerRiskScore(ctx context.Context, attackerIP string, score int, level string) error
}
...
func (p *Processor) process(
	...
	// Compute risk score on disconnect (session ended)
	if ev.EventType == types.EventDisconnect {
		p.computeRisk(ctx, ev.SourceIP)
	}
}
...
func (p *Processor) computeRisk(
	ctx context.Context, attackerIP string,
) {
	if p.scorer == nil || p.store == nil {
		return
	}

	// Get attacker profile
	profile, err := p.store.GetAttackerRiskProfile(ctx, attackerIP)
	if err != nil {
		p.logger.Error().
			Err(err).
			Str("ip", attackerIP).
			Msg("failed to get attacker profile for risk scoring")
		return
	}

	// Compute risk score
	riskScore := p.scorer.ComputeScore(profile)

	// Save full risk score with breakdown
	if err := p.store.SaveRiskScore(ctx, attackerIP, riskScore, profile); err != nil {
		p.logger.Error().
			Err(err).
			Str("ip", attackerIP).
			Msg("failed to save risk score")
	}

	// Update attacker record with current score
	if err := p.store.UpdateAttackerRiskScore(ctx, attackerIP, riskScore.Score, riskScore.Level); err != nil {
		p.logger.Error().
			Err(err).
			Str("ip", attackerIP).
			Msg("failed to update attacker risk score")
	}

	p.logger.Info().
		Str("ip", attackerIP).
		Int("score", riskScore.Score).
		Str("level", riskScore.Level).
		Msg("computed risk score")
}
