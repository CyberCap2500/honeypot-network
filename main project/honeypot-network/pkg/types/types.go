...
Shared domain types for the MirageNet deception network
...
// ──────────────────────────────────────────────────────────────────────
// Risk Scoring Types (NEW - Phase 1)
// ──────────────────────────────────────────────────────────────────────

// RiskFactor represents a single component of the risk calculation
type RiskFactor struct {
	Name        string  `json:"name"`        // e.g. "failed_auth_attempts"
	Value       float64 `json:"value"`       // raw metric value
	Weight      float64 `json:"weight"`      // 0.0-1.0 multiplier
	Score       float64 `json:"score"`       // weighted contribution (0-100)
	Description string  `json:"description"` // human-readable explanation
}

// RiskScore represents the final computed risk assessment (0-100)
type RiskScore struct {
	Score      int          `json:"score"`       // 0-100 final score
	Level      string       `json:"level"`       // "low", "medium", "high", "critical"
	Factors    []RiskFactor `json:"factors"`     // breakdown of contributing factors
	ComputedAt time.Time    `json:"computed_at"` // when this score was calculated
}

// AttackerRiskProfile holds all risk-relevant data for an IP address
type AttackerRiskProfile struct {
	SourceIP             string    `json:"source_ip"`
	SessionCount         int       `json:"session_count"`
	FailedAuthAttempts   int       `json:"failed_auth_attempts"`
	SuccessfulAuthCount  int       `json:"successful_auth_count"`
	CommandCount         int       `json:"command_count"`
	...
	EscalationSpeed      float64   `json:"escalation_speed"`       // commands per minute
}
